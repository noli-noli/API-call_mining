...
cleanware_sequences = cleanware_sequences.apply(lambda x: x[0:500])
new_data = dataset_preprocess(cleanware_sequences)

print(new_data.max())

X,y = create_dataset(new_data,False)
...
model = Sequential()
model.add(Embedding(302, 500, input_length=1000))
model.add(LSTM(64))
...
model.fit(X_train, y_train, batch_size=32,epochs=50, verbose=1,callbacks=[early_stopping])
model.save("500sequence.h5")
